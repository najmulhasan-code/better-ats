...
interface CandidateSourcingAnimationProps {
  frame: number;
  fps: number;
}
...
export const CandidateSourcingAnimation: React.FC<CandidateSourcingAnimationProps> = ({ frame, fps }) => {
  ...
  // Career site
  const careerSiteX = 200;
  const careerSiteY = 240;
      ...
      <svg width="100%" height="100%" viewBox="0 0 400 300" preserveAspectRatio="xMidYMid meet">
        ...
        <g clipPath="url(#sourcing-clip)">
          ...
          <g transform={`translate(${jobX}, ${jobY})`}>
            ...
            {/* Multi-board indicator */}
            <circle cx="0" cy="-40" r="8" fill="#10B981" opacity="0.8">
              <animate attributeName="opacity" values="0.8;0.3;0.8" dur="2s" repeatCount="indefinite" />
            </circle>
            <text x="0" y="-45" textAnchor="middle" fontSize="8" fill="#10B981" fontWeight="bold">50+</text>
          </g>
          ...
          {/* Career site builder */}
          <g transform={`translate(${careerSiteX}, ${careerSiteY})`} opacity={spring({ frame: frame - 20, fps, config: { damping: 200, stiffness: 200 } })}>
          ...
          {/* Automated sourcing indicator */}
          <g transform="translate(320, 240)" opacity={spring({ frame: frame - 30, fps, config: { damping: 200, stiffness: 200 } })}>
            <circle cx="0" cy="0" r="12" fill="#8B5CF6" opacity="0.2" />
            <text x="0" y="5" textAnchor="middle" fontSize="12" fill="#8B5CF6" fontWeight="bold">🔍</text>
            <text x="0" y="20" textAnchor="middle" fontSize="8" fill="#8B5CF6" fontWeight="semibold">Auto</text>
          </g>
        </g>
      </svg>
    </div>
